...
ssh "$PI_USER@$HOST" "sudo chmod 777 /var/kibana/data" || true
ssh "$PI_USER@$HOST" "sudo mkdir -p /var/kibana/config" || true
ssh "$PI_USER@$HOST" "sudo chmod 777 /var/kibana/config" || true
...
# ssh "$PI_USER@$HOST" "docker run -d --network host --restart unless-stopped --name $DOCKER_TAG \"$DOCKER_TAG\""
